...
pub use rs_vips::enums::Interpretation;
...
impl Image {
     ...
     * This may be overridden by other Image operations such as `toColourspace('b-w')`,
     * which will produce an output image containing one colour channel.
     * An alpha channel may be present, and will be unchanged by the operation.
     *
     * @example
     * const output = await Image(input).greyscale().toBuffer();
     *
    ...
    pub fn pipeline_colourspace(mut self, colourspace: Interpretation) -> Self {
        self.options.colourspace_pipeline = Some(colourspace);
        self
    }
    ...
    pub fn to_colourspace(mut self, colourspace: Interpretation) -> Self {
        self.options.colourspace = Some(colourspace);
        self
    }
}
