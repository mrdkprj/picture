...
use crate::image::{
    ...
    input::{create_input_descriptor, CreateRaw, Input, Inputs, MixedInput, RotateOptions, SharpInput, SharpOptions},
...
use rs_vips::{enums::OperationBoolean, Vips};
...
impl Image {
    fn init() -> Result<(), String> {
        Vips::init("pic_image").map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn new_sharp_from_inputs(inputs: Inputs, options: Option<SharpOptions>) -> Result<Self, String> {
        if inputs.inner.len() <= 1 {
        ...
        Self::init()?;
        let mut baton = init_options();
        baton.input = create_input_descriptor(SharpInput::Mixed(inputs.inner), options, &mut baton)?;
        Ok(Self {
            options: baton,
        })
    }
    ...
    fn new_sharp_from_file<P: AsRef<Path>>(filename: P, options: Option<SharpOptions>) -> Result<Self, String> {
        Self::init()?;
        let mut baton = init_options();
        baton.input = create_input_descriptor(SharpInput::Single(MixedInput::path(filename)), options, &mut baton)?;
        Ok(Self {
            options: baton,
        })
    }
    ...
    fn new_sharp_from_files<P: AsRef<Path>>(files: &[P], options: Option<SharpOptions>) -> Result<Self, String> {
        ...
        baton.input = create_input_descriptor(SharpInput::Mixed(inputs), options.clone(), &mut baton)?;
    ...
    fn new_sharp_from_buffer(buffer: Vec<u8>, options: Option<SharpOptions>) -> Result<Self, String> {
        Self::init()?;
        let mut baton = init_options();
        baton.input = create_input_descriptor(SharpInput::Single(MixedInput::Buffer(buffer)), options.clone(), &mut baton)?;
     ...
     *
     * @example
     * let rotateThenResize = await Image(input)
     *   .rotate(90)
     *   .resize({ width: 16, height: 8, fit: "fill" })
     *   .toBuffer();
     * let resizeThenRotate = await Image(input)
     *   .resize({ width: 16, height: 8, fit: "fill" })
     *   .rotate(90)
     *   .toBuffer();
     *
     ...
     *
     * @example
     * let output = await Image(input).autoOrient().toBuffer();
     *
     * @example
     * let pipeline = Image()
     *   .autoOrient()
     ...
     *
     * @example
     * let output = await Image(input).flip().toBuffer();
     */
     ...
     *
     * @example
     * let output = await Image(input).flop().toBuffer();
     */
     ...
     * A particular interpolator may also be specified. Set the `interpolator` option to an attribute of the `Image.interpolators` Object e.g. `Image.interpolators.nohalo`.
     *
     ...
     *      background: "white",
     *      interpolator: Image.interpolators.nohalo
     *   })
     ...
     *
     * @example
     * let data = await Image(input).sharpen().toBuffer();
     *
     * @example
     * let data = await Image(input).sharpen({ sigma: 2 }).toBuffer();
     *
     * @example
     ...
     *
     * @example
     * let output = await Image(input).median().toBuffer();
     *
     * @example
     * let output = await Image(input).median(5).toBuffer();
     */
     ...
     *
     * @example
     * let boxBlurred = await Image(input)
     *   .blur()
     *   .toBuffer();
     *
     * @example
     * let gaussianBlurred = await Image(input)
     *   .blur(5)
     *   .toBuffer();
     */
    pub fn blur(mut self, options: Option<BlurOptions>) -> Result<Self, String> {
        if let Some(options) = options {
            if !in_range(options.sigma, 0.0, 1000.0) {
                return Err(InvalidParameterError!("options.sigma", "number between 0.0 and 1000", sigma));
            }
    ...
    pub fn boolean(mut self, operand: Input, operator: OperationBoolean, options: Option<BooleanOptions>) -> Result<Self, String> {
        ...
        self.options.boolean_descriptor = Some(create_input_descriptor(SharpInput::Single(operand.inner), Some(sharp_options), &mut self.options)?);

        self.options.boolean_op = Some(operator);
