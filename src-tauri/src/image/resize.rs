use crate::image::{common::Canvas, in_range, Colour, Image, InvalidParameterError};
use num_derive::{FromPrimitive, ToPrimitive};
use rs_vips::enums::{Extend, Kernel};
...
impl Image {
     ...
     * - `Image.position`: `top`, `right top`, `right`, `right bottom`, `bottom`, `left bottom`, `left`, `left top`.
     * - `Image.gravity`: `north`, `northeast`, `east`, `southeast`, `south`, `southwest`, `west`, `northwest`, `center` or `centre`.
     * - `Image.strategy`: `cover` only, dynamically crop using either the `entropy` or `attention` strategy.
     *
     ...
     *     height: 200,
     *     fit: Image.fit.cover,
     *     position: Image.strategy.entropy
     *   });
     ...
     *
     * @example
     * const scaleByHalf = await Image(input)
     *   .metadata()
     *   .then(({ width }) => Image(input)
     ...
     * const output = await Image(input)
     *   .trim({
     *     background: "#FF0000",
     *     lineArt: true
     *   })
