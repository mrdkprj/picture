...
impl Image {
    ...
    pub fn set_cache(memory: u64, files: i32, items: i32) -> CacheResult {
        // Set memory limit
        Vips::cache_set_max_mem(memory as _);
        // Set file limit
        ...
        // Get memory stats
        result.memory.current = Vips::tracked_get_mem() as u64 / 1048576;
        result.memory.high = Vips::tracked_get_mem_highwater() as u64 / 1048576;
        result.memory.max = Vips::cache_get_max_mem() as u64 / 1048576;
...
pub(crate) fn get_g_type(name: &str) -> usize {
...
pub(crate) fn available_formats() -> HashMap<String, AvailableFormat> {
    Vips::init("Image").unwrap();
    let mut formats = HashMap::new();
    ["jpeg", "png", "webp", "tiff", "magick", "openslide", "dz", "ppm", "fits", "gif", "svg", "heif", "pdf", "vips", "jp2k", "jxl", "rad", "dcraw"].into_iter().for_each(|f| {
        let basename = new_c_string("VipsOperation").unwrap();
        // input
        let load = new_c_string(&format!("{f}load")).unwrap();
        let has_input_file = !unsafe { vips_class_find(basename.as_ptr(), load.as_ptr()) }.is_null();
        let load_buffer = new_c_string(&format!("{f}load_buffer")).unwrap();
        let has_input_buffer = !unsafe { vips_class_find(basename.as_ptr(), load_buffer.as_ptr()) }.is_null();
        ...
        let has_output_buffer = unsafe { vips_type_find(basename.as_ptr(), save_buffer.as_ptr()) } > 0;
