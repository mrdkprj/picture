use crate::image::{
    common::{
        apply_alpha, assert_image_type_dimensions, calculate_crop, calculate_crop2, calculate_embed_position, ensure_alpha, exif_orientation, get_profile, has_gain_map, has_profile, image_type_id,
        ...
        remove_gain_map, remove_gif_palette, resolve_shrink, set_animation_properties, set_density, set_exif_orientation, set_profile, set_timeout, stay_sequential, Canvas, ImageType,
        InputDescriptor,
    },
    input::open_input,
    operation::{
        bandbool, blur, boolean, clahe, convolve, crop_multi_page, dilate, embed_multi_page, ensure_colourspace, erode, flatten, foreign_webp_preset_string, gamma, linear, modulate, negate,
        normalise, recomb, sharpen, threshold, tint, trim, unflatten,
    },
    util::{get_g_type, VipsGuard, G_TYPE_INT},
};
use rs_vips::{
    bindings::{VipsForeignKeep_VIPS_FOREIGN_KEEP_EXIF, VipsForeignKeep_VIPS_FOREIGN_KEEP_ICC, VipsForeignKeep_VIPS_FOREIGN_KEEP_XMP, VIPS_META_N_PAGES, VIPS_META_PAGE_HEIGHT, VIPS_META_XMP_NAME},
    enums::{
...
pub(crate) struct PipelineBaton {
    ...
    pub(crate) with_gain_map: bool,
    pub(crate) keep_gain_map: bool,
    ...
    pub(crate) boolean_op: Option<OperationBoolean>,
    pub(crate) band_bool_op: Option<OperationBoolean>,
    pub(crate) extract_channel: i32,
    pub(crate) remove_alpha: bool,
    pub(crate) ensure_alpha: f64,
    pub(crate) colourspace_pipeline: Option<Interpretation>,
    pub(crate) colourspace: Option<Interpretation>,
    ...
    pub(crate) tile_depth: Option<ForeignDzDepth>,
...
pub(crate) fn init_options() -> PipelineBaton {
    PipelineBaton {
        ...
        remove_alpha: false,
        ensure_alpha: -1.0,
        colourspace: Some(Interpretation::Srgb),
        colourspace_pipeline: None,
        composite: Vec::new(),
        // output
        file_out: String::new(),
...
pub(crate) fn pipline(mut baton: PipelineBaton) -> Result<PipelineBaton> {
    ...
    // Rotate pre-extract
    let should_rotate_before = baton.rotate_before && (rotation != Angle::D0 || baton.flip || baton.flop || baton.rotation_angle != 0.0);
    let should_orient_before = (should_rotate_before || baton.orient_before) && (auto_rotation != Angle::D0 || auto_flop);
    ...
    if should_rotate_before {
        image = stay_sequential(image, rotation != Angle::D0 || baton.flip || baton.rotation_angle != 0.0)?;
        ...
        if baton.rotation_angle != 0.0 {
            ...
            image = alpha_image;
            image = image.rotate_with_opts(baton.rotation_angle, VOption::new().set("background", background.as_slice()))?;
    ...
    let should_pre_shrink = (target_resize_width > 0 || target_resize_height > 0)
        ...
        && !baton.keep_gain_map
        && !baton.with_gain_map
        && baton.colourspace_pipeline.is_none()
    ...
    let mut image = if jpeg_shrink_on_load > 1 {
        let option = VOption::new().set("access", access as i32).set("shrink", jpeg_shrink_on_load).set("unlimited", baton.input.unlimited).set("fail_on", baton.input.fail_on as i32);
        ...
        if input_image_type == ImageType::WEBP {
            let option = VOption::new().set("access", access as i32).set("scale", scale).set("fail_on", baton.input.fail_on as i32).set("n", baton.input.pages).set("page", baton.input.page);
            ...
            let option =
                VOption::new().set("access", access as i32).set("scale", scale).set("fail_on", baton.input.fail_on as i32).set("unlimited", baton.input.unlimited).set("dpi", baton.input.density);
            ...
            if image.get_width() > 32767 || image.get_height() > 32767 {
                return Err(OperationError("Input SVG image will exceed 32767x32767 pixel limit when scaled".to_string()));
            }
            image
        } else if input_image_type == ImageType::PDF {
            let option = VOption::new().set("n", baton.input.pages).set("page", baton.input.page).set("dpi", baton.input.density).set("background", baton.input.pdf_background.as_slice());
        ...
        if input_image_type == ImageType::SVG && (image.get_width() > 32767 || image.get_height() > 32767) {
            return Err(OperationError("Input SVG image exceeds 32767x32767 pixel limit".to_string()));
        }
        image
    };
    ...
    let mut gain_map = VipsImage::new();
    let mut gainmap_scale_factor = 1;
    ...
    if has_gain_map(&image) {
        ...
        } else if baton.with_gain_map {
            image = image.uhdr2scRGB()?;
        }
        image = remove_gain_map(image)?;
    } else {
        baton.keep_gain_map = false;
        baton.with_gain_map = false
    }
    ...
    if has_profile(&image)
        ...
        && (baton.colourspace_pipeline.is_none() || (baton.colourspace_pipeline.is_some() && baton.colourspace_pipeline.unwrap() != Interpretation::Cmyk))
        ...
        && !baton.with_gain_map
    {
        ...
        && (baton.colourspace_pipeline.is_none() || (baton.colourspace_pipeline.is_some() && baton.colourspace_pipeline.unwrap() != Interpretation::Cmyk))
    {
        image = image.icc_transform_with_opts(processing_profile, VOption::new().set("input_profile", "cmyk").set("intent", Intent::Perceptual as i32))?;
    }
    ...
    let should_premultiply_alpha = image.hasalpha() && (should_resize || should_blur || should_conv || should_sharpen);
    ...
    // Resize
    if should_resize {
        image = image.resize_with_opts(1.0 / hshrink, VOption::new().set("vscale", 1.0 / vshrink).set("kernel", baton.kernel as i32))?;

        if baton.keep_gain_map {
            gain_map = gain_map.resize_with_opts(1.0 / hshrink, VOption::new().set("vscale", 1.0 / vshrink).set("kernel", baton.kernel as i32))?;
        }
    }

    image = stay_sequential(image, auto_rotation != Angle::D0 || baton.flip || rotation != Angle::D0)?;
    ...
    if auto_rotation != Angle::D0 {
        ...
        image = image.rot(auto_rotation)?;
        if baton.keep_gain_map {
            gain_map = gain_map.rot(auto_rotation)?;
        }
    }
    ...
    if baton.flip {
        image = image.flip(Direction::Vertical)?;
        if baton.keep_gain_map {
            gain_map = gain_map.flip(Direction::Vertical)?;
        }
    }
    ...
    if baton.flop != auto_flop {
        image = image.flip(Direction::Horizontal)?;
        if baton.keep_gain_map {
            gain_map = gain_map.flip(Direction::Horizontal)?;
        }
    }
    ...
    if rotation != Angle::D0 {
        ...
        image = image.rot(rotation)?;
        if baton.keep_gain_map {
            gain_map = gain_map.rot(rotation)?;
        }
    }
    ...
    if !baton.join_channel_in.is_empty() {
        ...
        if let Some(colourspace) = baton.colourspace {
            image = image.copy_with_opts(VOption::new().set("interpretation", colourspace as i32))?;
        }
    ...
    if input_width != baton.width || input_height != baton.height {
        if baton.canvas == Canvas::Embed {
            ...
            image = if n_pages > 1 {
                ...
                image.embed_with_opts(left, top, width, height, VOption::new().set("extend", Extend::Background as i32).set("background", background.as_slice()))?
            };
            if baton.keep_gain_map {
                gain_map = gain_map.embed_with_opts(
                    left / gainmap_scale_factor,
                    top / gainmap_scale_factor,
                    width / gainmap_scale_factor,
                    height / gainmap_scale_factor,
                    VOption::new().set("extend", Extend::Background as i32).set("background", 0),
                )?;
            }
            ...
            if baton.position < 9 {
                ...
                if baton.keep_gain_map {
                    gain_map = gain_map.extract_area(left / gainmap_scale_factor, top / gainmap_scale_factor, width / gainmap_scale_factor, height / gainmap_scale_factor)?;
                }
            } else {
                // Attention-based or Entropy-based crop
                multi_page_unsupported(n_pages, "Resize strategy")?;
                keep_gain_map_unsupported(baton.keep_gain_map, "Resize strategy")?;
                image = stay_sequential(image, true)?;
                ...
                baton.attention_x = (attention_x as f64 * jpeg_shrink_on_load as f64 / scale) as i32;
                baton.attention_y = (attention_y as f64 * jpeg_shrink_on_load as f64 / scale) as i32;
            }
        }
    }
    ...
    if !baton.rotate_before && baton.rotation_angle != 0.0 {
        ...
        image = alpha_image.rotate_with_opts(baton.rotation_angle, VOption::new().set("background", background.as_slice()))?;
        if baton.keep_gain_map {
            gain_map = gain_map.rotate_with_opts(baton.rotation_angle, VOption::new().set("background", 0))?;
        }
    }
    ...
    if baton.top_offset_post != -1 {
        if n_pages > 1 {
            ...
            if baton.keep_gain_map {
                gain_map = gain_map.extract_area(
                    baton.left_offset_post / gainmap_scale_factor,
                    baton.top_offset_post / gainmap_scale_factor,
                    baton.width_post / gainmap_scale_factor,
                    baton.height_post / gainmap_scale_factor,
                )?;
            }
        }
    }
    ...
    if !baton.affine_matrix.is_empty() {
        ...
        keep_gain_map_unsupported(baton.keep_gain_map, "Affine")?;
        image = stay_sequential(image, true)?;
        ...
        if baton.extend_with == Extend::Background {
            ...
            image = if n_pages > 1 {
                ...
                image.embed_with_opts(
                    ...
                    baton.width,
                    baton.height,
                    VOption::new().set("extend", baton.extend_with as i32).set("background", background.as_slice()),
                )?
            };
            if baton.keep_gain_map {
                gain_map = gain_map.embed_with_opts(
                    baton.extend_left / gainmap_scale_factor,
                    baton.extend_top / gainmap_scale_factor,
                    baton.width / gainmap_scale_factor,
                    baton.height / gainmap_scale_factor,
                    VOption::new().set("extend", baton.extend_with as i32).set("background", 0),
                )?;
            }
        } else {
            ...
            image = if n_pages > 1 {
                ...
                image.embed_with_opts(baton.extend_left, baton.extend_top, baton.width, baton.height, VOption::new().set("extend", baton.extend_with as i32))?
            };
            if baton.keep_gain_map {
                gain_map = gain_map.embed_with_opts(
                    baton.extend_left / gainmap_scale_factor,
    ...
    if should_blur {
        ...
        if baton.keep_gain_map {
            gain_map = blur(gain_map, baton.blur_sigma, baton.precision, baton.min_ampl)?;
        }
    }
    ...
    // Convolve
    if should_conv {
        keep_gain_map_unsupported(baton.keep_gain_map, "Convolve")?;
    ...
    // Composite
    if should_composite {
        keep_gain_map_unsupported(baton.keep_gain_map, "Composite")?;
        let mut images = Vec::new();
        ...
        for composite in baton.composite.as_mut_slice() {
            ...
            if composite.input.auto_orient {
                ...
                let (composite_auto_rotation, composite_auto_flop) = calculate_exif_rotation_and_flop(exif_orientation(&composite_image));

                composite_image = remove_exif_orientation(composite_image)?;
                composite_image = stay_sequential(composite_image, composite_auto_rotation != Angle::D0)?;
            ...
            if composite_image.get_width() > image.get_width() || composite_image.get_height() > image.get_height() {
                return Err(OperationError("Image to composite must have same dimensions or smaller".to_string()));
            }
        ...
        image = VipsImage::composite_with_opts(
            ...
            VOption::new()
                .set(
                    "compositing_space",
                    if let Some(colourspace_pipeline) = baton.colourspace_pipeline {
                        colourspace_pipeline
                    } else {
                        Interpretation::Srgb
                    } as i32,
                )
    ...
    if baton.normalise {
        keep_gain_map_unsupported(baton.keep_gain_map, "Normalise")?;
        image = stay_sequential(image, true)?;
    ...
    if baton.clahe_width != 0 && baton.clahe_height != 0 {
        keep_gain_map_unsupported(baton.keep_gain_map, "Clahe")?;
        image = stay_sequential(image, true)?;
    ...
    if let Some(boolean_descriptor) = baton.boolean_descriptor.as_mut() {
        keep_gain_map_unsupported(baton.keep_gain_map, "Boolean")?;
        ...
        if let Some(boolean_op) = baton.boolean_op {
            image = boolean(image, &boolean_image, boolean_op)?;
        }
        image = remove_gif_palette(image)?;
    }

    // Apply per-channel Bandbool bitwise operations after all other operations
    if let Some(band_bool_op) = baton.band_bool_op {
        if band_bool_op >= OperationBoolean::And {
            image = bandbool(image, band_bool_op)?;
        }
    }
    ...
    if let Some(colourspace) = baton.colourspace {
        if image.get_interpretation()? != colourspace {
            // Convert colourspace, pass the current known interpretation so libvips doesn't have to guess
            image = image.colourspace_with_opts(colourspace, VOption::new().set("source_space", image.get_interpretation()? as i32))?;
    ...
    if baton.extract_channel > -1 {
        keep_gain_map_unsupported(baton.keep_gain_map, "Extract channel")?;
        ...
        image = image.copy_with_opts(VOption::new().set("interpretation", colourspace_value as i32))?;
    }
    ...
    if baton.negate {
        ...
        if baton.keep_gain_map {
            gain_map = negate(gain_map, false)?;
        }
    }
    ...
    // XMP buffer
    if (baton.keep_metadata as u32 & VipsForeignKeep_VIPS_FOREIGN_KEEP_XMP != 0) && !baton.with_xmp.is_empty() {
        image = image.copy()?;
        image.set_blob_copy(VIPS_META_XMP_NAME, baton.with_xmp.as_bytes())?;
    }
    ...
    if baton.file_out.is_empty() {
        write_to_buffer(image, input_image_type, baton, gain_map, gainmap_scale_factor)
    } else {
        write_to_file(image, input_image_type, baton, gain_map, gainmap_scale_factor)
    }
}

fn write_to_buffer(mut image: VipsImage, input_image_type: ImageType, mut baton: PipelineBaton, gain_map: VipsImage, gainmap_scale_factor: i32) -> Result<PipelineBaton> {
    // Buffer output
    ...
    if baton.format_out == "jpeg" || (baton.format_out == "input" && input_image_type == ImageType::JPEG) {
        ...
        let area = if baton.keep_gain_map {
            image = reattach_gain_map(image, gain_map, &baton)?;
            image.uhdrsave_buffer_with_opts(VOption::new().set("keep", baton.keep_metadata).set("Q", baton.jpeg_quality).set("gainmap_scale_factor", gainmap_scale_factor))?
        } else {
            image.jpegsave_buffer_with_opts(
                VOption::new()
        ...
        if baton.colourspace.is_some() && baton.colourspace.unwrap() == Interpretation::Cmyk {
        ...
        if let Some(tile_depth) = baton.tile_depth {
            options.add("depth", tile_depth as i32);
        }
...
fn write_to_file(mut image: VipsImage, input_image_type: ImageType, mut baton: PipelineBaton, gain_map: VipsImage, gainmap_scale_factor: i32) -> Result<PipelineBaton> {
    // File output
    set_timeout(&image, baton.timeout_seconds);
    ...
    if baton.format_out == "jpeg" || (might_match_input && is_jpeg) || (will_match_input && input_image_type == ImageType::JPEG) {
        ...
        if baton.keep_gain_map {
            image = reattach_gain_map(image, gain_map, &baton)?;
            image.uhdrsave_with_opts(&baton.file_out, VOption::new().set("keep", baton.keep_metadata).set("Q", baton.jpeg_quality).set("gainmap_scale_factor", gainmap_scale_factor))?;
        } else {
        ...
        // Write V to file
        image.vipssave_with_opts(&baton.file_out, VOption::new().set("keep", baton.keep_metadata))?;
...
fn keep_gain_map_unsupported(keep_gain_map: bool, op: &str) -> Result<()> {
    if keep_gain_map {
        Err(OperationError(format!("{op} is not supported when keeping gain maps")))
    } else {
        Ok(())
    }
}
...
fn reattach_gain_map(image: VipsImage, gain_map: VipsImage, baton: &PipelineBaton) -> Result<VipsImage> {
    let gain_map_jpeg = gain_map.jpegsave_buffer_with_opts(
        VOption::new()
            .set("keep", false)
            .set("Q", baton.jpeg_quality)
            .set("subsample_mode", ForeignSubsample::Off as i32)
            .set("trellis_quant", baton.jpeg_trellis_quantisation)
            .set("quant_table", baton.jpeg_quantisation_table)
            .set("overshoot_deringing", baton.jpeg_overshoot_deringing)
            .set("optimize_coding", baton.jpeg_optimise_coding),
    ...
    let mut image = image.copy()?;
    image.set_blob_copy("gainmap-data", &gain_map_jpeg)?;
