use crate::image::{
    input::{create_input_descriptor, Create, CreateRaw, CreateText, Input, SharpInput, SharpOptions},
    pipeline::Composite,
    resize::Gravity,
    Image, InvalidParameterError,
};
use rs_vips::enums::{BlendMode, FailOn};
...
pub struct OverlayOptions {
    ...
    /** see Image() constructor, (optional, default 'warning') */
    pub fail_on: Option<FailOn>,
    /** see Image() constructor, (optional, default 268402689) */
    pub limit_input_pixels: Option<usize>,
    /** see Image() constructor, (optional, default false) */
...
impl Image {
    pub fn composite(mut self, images: &[OverlayOptions]) -> Result<Self, String> {
        ...
        for image in images {
            let descriptor = create_input_descriptor(SharpInput::Single(image.input.inner.clone()), Some(create_sharp_options(image)), &mut self.options)?;
