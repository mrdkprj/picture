...
pub(crate) fn get_metadata(input: &InputDescriptor) -> Result<Metadata> {
    ...
    if image_type != ImageType::UNKNOWN {
        ...
        if image.get_typeof("jpeg-chroma-subsample")? == unsafe { vips_ref_string_get_type() } {
            baton.chroma_subsampling = image.get_string("jpeg-chroma-subsample").unwrap_or_default();
        }
        ...
        if image.get_typeof("openslide.level-count")? == unsafe { vips_ref_string_get_type() } {
            let levels: i32 = image.get_string("openslide.level-count").unwrap_or(String::from("0")).parse().unwrap();
            for l in 0..levels {
                let prefix = format!(r#"openslide.level["{:?}"]."#, l);
                let width: i32 = image.get_string(format!("{}width", prefix)).unwrap_or(String::from("0")).parse().unwrap();
                let height: i32 = image.get_string(format!("{}height", prefix)).unwrap_or(String::from("0")).parse().unwrap();
        ...
        // Gain Map
        if image.get_typeof("gainmap-data")? == unsafe { vips_blob_get_type() } {
            baton.gain_map = image.get_blob("gainmap-data").unwrap_or_default();
        }
        // PNG comments
...
unsafe extern "C" fn read_pngcomment(image: *mut rs_vips::bindings::_VipsImage, field: *const c_char, _value: *mut GValue, data: *mut c_void) -> *mut c_void {
    ...
    if vips_isprefix(png_comment_start.as_ptr(), field) == 1 {
        ...
        if let Some(keyword) = keyword {
            if vips_image_get_string(image, field, &mut str) == 0 {
                // Skip the hyphen
