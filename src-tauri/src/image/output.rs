...
use crate::image::{
    ...
    Colour, Image, InvalidParameterError,
};
use num_derive::{FromPrimitive, ToPrimitive};
use rs_vips::{
    bindings::vips_enum_nick,
    enums::{
        BandFormat, ForeignDzContainer, ForeignDzDepth, ForeignDzLayout, ForeignHeifCompression, ForeignTiffCompression, ForeignTiffPredictor, ForeignTiffResunit, ForeignWebpPreset, Interpretation,
    },
};
...
impl Image {
    ...
    fn create_output_info(baton: PipelineBaton) -> OutputInfo {
        ...
        OutputInfo {
            ...
            depth: if baton.format_out == "raw" {
                unsafe { vips_enum_nick(rs_vips::bindings::vips_band_format_get_type(), baton.raw_depth as i32) as i32 }
            } else {
                0
            },
     ...
     *
     * @example
     * let outputWithExif = await Image(inputWithExif)
     *   .keepExif()
     *   .toBuffer();
     *
     * @returns {Image}
     ...
     *
     * @example
     * let dataWithExif = await Image(input)
     *   .withExif({
     *     IFD0: {
     ...
     *
     * @example
     * let dataWithMergedExif = await Image(inputWithExif)
     *   .withExifMerge({
     *     IFD0: {
     *       Copyright: "The National Gallery"
     *     }
     ...
     *
     * @example
     * let outputWithIccProfile = await Image(inputWithIccProfile)
     *   .keepIccProfile()
     *   .toBuffer();
     *
     * @returns {Image}
     ...
     *
     * @example
     * let outputWithP3 = await Image(input)
     *   .withIccProfile("p3")
     *   .toBuffer();
     *
     ...
     *
     * @example
     * const outputWithResizedGainMap = await Image(inputWithGainMap)
     *   .keepGainMap()
     *   .resize({ width: 64 })
     *   .toBuffer();
     *
     * @returns {Image}
     */
    pub fn keep_gain_map(mut self) -> Self {
        self.options.keep_gain_map = true;
        self.options.with_gain_map = false;
        self.options.keep_metadata |= 0b100000;
        self
    }
     ...
     *
     * @example
     * const outputWithRegeneratedGainMap = await Image(inputWithGainMap)
     *   .withGainMap()
     *   .toBuffer();
     *
     * @returns {Image}
     */
    pub fn with_gain_map(mut self) -> Self {
        self.options.with_gain_map = true;
        self.options.keep_gain_map = false;
        self.options.colourspace = Some(Interpretation::Scrgb);
        self
    }
     ...
     *
     * @example
     * const outputWithXmp = await Image(inputWithXmp)
     *   .keepXmp()
     *   .toBuffer();
     *
     * @returns {Image}
     ...
     *
     * @example
     * let outputWithMetadata = await Image(inputWithMetadata)
     *   .keepMetadata()
     *   .toBuffer();
     *
     * @returns {Image}
     ...
     *
     * @example
     * let outputSrgbWithMetadata = await Image(inputRgbWithMetadata)
     *   .withMetadata()
     *   .toBuffer();
     *
     * @example
     * // Set output metadata to 96 DPI
     * let data = await Image(input)
     ...
     * let outputWebp = await Image(inputWebp, { animated: true })
     *   .webp({ effort: 6 })
     *   .toBuffer();
     *
     */
     ...
     * @example
     * // Convert PNG to GIF
     * await Image(pngBuffer)
     *   .gif()
     *   .toBuffer();
     *
     * @example
     * // Convert animated WebP to animated GIF
     * await Image("animated.webp", { animated: true })
     *   .toFile("animated.gif");
     *
     * @example
     * // Create a 128x128, cropped, non-dithered, animated thumbnail of an animated GIF
     * let out = await Image("in.gif", { animated: true })
     ...
     * await Image("in.gif", { animated: true })
     ...
     * {@link https://Image.pixelplumbing.com/install#custom-libvips installing a custom libvips}.
     *
     * @example
     ...
     */
    fn try_set_animation_options(mut self, loop_: Option<i32>, delay: Option<Vec<i32>>) -> Result<Self, String> {
     ...
     * let { data, info } = await Image("input.jpg")
     *   .raw()
     *   .toBuffer({ resolveWithObject: true });
     *
     * @example
     * // Extract alpha channel as raw, unsigned 16-bit pixel data from PNG input
     * let data = await Image("input.png")
     *   .ensureAlpha()
     *   .extractChannel(3)
     *   .toColourspace("b-w")
     *   .raw({ depth: "ushort" })
     ...
     *
     * @example
     * let zipFileWithTiles = await Image(input)
     *   .tile({ basename: "tiles" })
     *   .toBuffer();
     *
     * @example
     * let iiififier = Image().tile({ layout: "iiif" });
     * readableStream
     *   .pipe(iiififier)
    ...
    pub fn tile(mut self, options: Option<TileOptions>) -> Result<Self, String> {
        ...
        if let Some(options) = options {
            ...
            // Depth of tiles
            self.options.tile_depth = options.depth;
