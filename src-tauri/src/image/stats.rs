...
impl Image {
    pub fn stats(&self) -> Result<Stats> {
        ...
        if image.get_width() > 1 || image.get_height() > 1 {
            let mut laplacian = VipsImage::new_matrixv(3, 3, &[0.0, 1.0, 0.0, 1.0, -4.0, 1.0, 0.0, 1.0, 0.0])?;
        ...
        let hist = remove_alpha(image)?.colourspace(Interpretation::Srgb)?.hist_find_ndim_with_opts(VOption::new().set("bins", 16))?;
