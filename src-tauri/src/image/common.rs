use crate::image::util::{get_g_type, new_c_string, G_TYPE_INT};
use rs_vips::{
    bindings::{
        ...
        vips_interpretation_max_alpha, vips_malloc, GValue, VIPS_META_ICC_NAME, VIPS_META_N_PAGES, VIPS_META_ORIENTATION, VIPS_META_PAGE_HEIGHT, VIPS_META_SEQUENTIAL,
    },
    enums::{Access, Align, BandFormat, FailOn, Interpretation, TextWrap},
...
pub(crate) fn determine_image_type(buffer: &[u8]) -> ImageType {
    unsafe {
        ...
        if let Ok(loader_name) = c_str.to_str() {
            let image_type = loader_to_type().get(loader_name).cloned().unwrap_or(ImageType::UNKNOWN);
            if image_type == ImageType::UHDR {
                ImageType::JPEG
            } else {
                image_type
            }
...
pub(crate) fn image_type_supports_unlimited(image_type: &ImageType) -> bool {
    image_type == &ImageType::JPEG || image_type == &ImageType::PNG || image_type == &ImageType::SVG || image_type == &ImageType::TIFF || image_type == &ImageType::HEIF
}

/*
  Should HEIF image be re-opened using the primary item?
*/
pub(crate) fn heif_primary_page_reopen(image: &VipsImage, descriptor: &mut InputDescriptor) -> Result<bool> {
    if image.get_typeof(VIPS_META_N_PAGES)? == get_g_type(G_TYPE_INT)
        && image.get_typeof("heif-primary")? == get_g_type(G_TYPE_INT)
        && image.get_int(VIPS_META_N_PAGES)? > 1
        && descriptor.pages == 1
        && descriptor.page == -1
    {
        let page_primary = image.get_int("heif-primary")?;
        if page_primary != 0 {
            descriptor.page = page_primary;
            return Ok(true);
        }
    }
    Ok(false)
...
pub(crate) fn set_profile(image: VipsImage, icc: Option<Vec<u8>>) -> Result<VipsImage> {
    if let Some(icc) = icc {
        ...
        image.set_blob_copy(VIPS_META_ICC_NAME, &icc)?;
        return Ok(image);
    }
    Ok(image)
}

unsafe extern "C" fn remove_exif_callback(_image: *mut rs_vips::bindings::_VipsImage, name: *const c_char, _value: *mut GValue, data: *mut c_void) -> *mut c_void {
...
*/
pub(crate) fn remove_exif(image: &mut VipsImage) {
...
pub(crate) fn set_animation_properties(image: VipsImage, n_pages: i32, page_height: i32, delay: &[i32], loop_: i32) -> Result<VipsImage> {
    ...
    let mut copied_image = image.copy()?;
    ...
    if has_delay {
        ...
        if delay.len() == 1 {
            ...
            delay.extend(std::iter::repeat_n(delay[0], (n_pages - 1) as usize));
        }
...
pub(crate) fn assert_image_type_dimensions(image: &VipsImage, image_type: ImageType) -> Result<()> {
    ...
    if image_type == ImageType::JPEG {
        if image.get_width() > 65535 || height > 65535 {
            return Err(OperationError("Processed image is too large for the JPEG format".to_string()));
        }
    } else if image_type == ImageType::WEBP {
        if image.get_width() > 16383 || height > 16383 {
            return Err(OperationError("Processed image is too large for the WebP format".to_string()));
        }
    } else if image_type == ImageType::GIF {
        if image.get_width() > 65535 || height > 65535 {
            return Err(OperationError("Processed image is too large for the GIF format".to_string()));
        }
        ...
        if seconds > 0 {
            ...
            if (*im).progress_signal.is_null() {
                ...
                *timeout = seconds as i32;
                ...
                g_signal_connect_data(im as *mut _, c"eval".as_ptr(), Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(vips_progress_call_back as *const ())), timeout as *mut _, None, 0);
...
*/
unsafe extern "C" fn vips_progress_call_back(im: *mut rs_vips::bindings::VipsImage, progress: *mut rs_vips::bindings::VipsProgress, timeout: *mut c_int) {
...
pub(crate) fn get_rgba_as_colourspace(rgba: Vec<f64>, interpretation: Interpretation, should_premultiply: bool) -> Result<Vec<f64>> {
    ...
    let mut pixel = VipsImage::new_matrix(1, 1)?;
    pixel.set_int("bands", bands as _)?;
    let pixel = VipsImage::new_from_image(&pixel, &rgba)?.colourspace_with_opts(interpretation, VOption::new().set("source_space", Interpretation::Srgb as i32))?;
...
pub(crate) fn apply_alpha(image: VipsImage, colour: &[f64], should_premultiply: bool) -> Result<(VipsImage, Vec<f64>)> {
    ...
    if colour[3] < 255.0 || image.hasalpha() {
        ...
        alpha_colour.extend(std::iter::repeat_n(colour[3] * multiplier, extra_bands as usize));
    }
...
pub(crate) fn stay_sequential(image: VipsImage, condition: bool) -> Result<VipsImage> {
    if unsafe { vips_image_is_sequential(image.as_mut_ptr()) > 0 } && condition {
        let mut copied_image = image.copy_memory()?.copy()?;
...
*/
pub(crate) fn has_gain_map(image: &VipsImage) -> bool {
    if let Ok(gtype) = image.get_typeof("gainmap") {
        if gtype == unsafe { vips_blob_get_type() } {
            return true;
        }

        if let Ok(gtype) = image.get_typeof("gainmap-data") {
            if gtype == unsafe { vips_blob_get_type() } {
                return true;
            }
        }
...
*/
pub(crate) fn remove_gain_map(image: VipsImage) -> Result<VipsImage> {
